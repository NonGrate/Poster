...
- Every key to obtain and where to paste it: `docs/Credentials.md` is the
  full list (server env, `gradle.properties`, `Local.xcconfig`, GitHub secrets);
  filter it to the features left on and hand over `server/.env.example` with
  the relevant lines uncommented.
