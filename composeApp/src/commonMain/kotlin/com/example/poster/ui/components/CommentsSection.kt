...
fun CommentsSection(
    ...
    Column(modifier = modifier.testTag("comments_section")) {
        ...
        Row(verticalAlignment = Alignment.Bottom) {
            // The weight goes on a wrapper, not on the field: on iOS the
            // adaptive field applies the caller's modifier to its inner
            // BasicTextField while its own frame fills the width, which pushed
            // the send button off the screen (see Adaptive.ios.kt).
