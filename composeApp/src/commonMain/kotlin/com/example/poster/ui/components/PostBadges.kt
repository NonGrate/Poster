    ...
    Surface(
        ...
        Row(
            ...
            Text(
                ...
                maxLines = 1,
                softWrap = false,
                // softWrap=false stops the label wrapping but its *minimum*
                // intrinsic width is still its longest word, and the card's
                // FlowRow fits this weighted control onto a line by that
                // minimum — so with a comment count beside it the line was
                // accepted with room for "Likes" and the "· 8" was clipped.
                // Pinning the width to the max intrinsic makes both agree.
                modifier = Modifier.width(IntrinsicSize.Max).testTag("favorite_count"),
            )
        }
    }
}
