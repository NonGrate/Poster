...
import poster.composeapp.generated.resources.likers_added_other
        ...
        BoxWithConstraints(modifier = Modifier.weight(1f).fillMaxWidth()) {
            ...
            PostCard(
                ...
                if (Features.LIKES && roster.total > 0) {
                    ...
                    LikedRoster(roster, ownPost = isOwnPost, modifier = Modifier.fillMaxWidth())
                }
...
@Composable
private fun LikedRoster(list: LikerList, ownPost: Boolean, modifier: Modifier = Modifier) {
...
@Composable
private fun LikerRow(person: Liker, ownPost: Boolean) {
    Row(
        ...
        Column {
            Text(
                // "your post" only when it is; a reader on somebody else's
                // post was told everyone had liked *their* post.
                text = stringResource(
                    if (ownPost) Res.string.likers_added else Res.string.likers_added_other,
                    person.name,
                ),
                style = MaterialTheme.typography.bodyMedium,
            )
