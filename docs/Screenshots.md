...
groups, a liked-by roster with named and anonymous people, and four comments
(a short exchange under the newest post, one under another) so the count chip
and the thread appear. The screenshot scripts sign in as the reader. The content is in four small functions
...
`store/` (the composed store tiles) is a git-ignored output; `screenshots/` is
committed because the README and the docs embed it. Commit store tiles too if
