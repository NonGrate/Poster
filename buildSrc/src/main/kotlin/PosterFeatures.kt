...
object PosterFeatures {
    ...
    PosterFeature("liquidDesign", "Liquid look", "Look", default = false, requires = listOf(), conflicts = listOf(),
        app = "Rounder shapes, translucent cards, and a floating capsule tab bar instead of the docked one, all drawn by Compose ([LiquidDesign.md](LiquidDesign.md))",
        server = "\u2014",
        notes = ""),
    PosterFeature("liquidNavBar", "Native iOS tab bar", "Look", default = true, requires = listOf(), conflicts = listOf(),
        app = "iOS uses the system's own tab bar — Liquid Glass on iOS 26, the ordinary bar before it — and the content scrolls behind it. Off, iOS draws the same docked bar as everywhere else ([LiquidDesign.md](LiquidDesign.md))",
        server = "\u2014",
        notes = "iOS only. It used to give Android, desktop and the browser a floating capsule as well, which is not what those platforms' people expect; that capsule now belongs to `liquidDesign`."),
