...
# The store-tile builder reads the per-language folder; the pages under
# screenshots/ and the docs read flat files, so the English set is also laid
# next to it (the folder itself is git-ignored).
[ "$SCREENSHOT_LANG" = en ] && cp "$CAPTURE_DIR"/*.png "$OUT"/
