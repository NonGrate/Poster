...
DETAIL_COMMENT=$("$SEEDER" --detail-comment "$DEMO_LANG")
...
capture_set() {  # capture_set <prefix>
  ...
  want 09-post-details 09b-post-comments && { capture_details "$t"     || echo "  (skipped post details)"; }
  ...
  # With --only, a final `want` that says no would otherwise be this function's
  # exit status, and set -e would end the run before the dark pass.
  return 0
}
...
capture_details() {
  ...
  # The liked-by roster pushes the comments below the fold on a phone. The
  # first comment's text is the anchor: "Comments" also names the count chip
  # on the card, which is on screen before any scrolling.
  scroll_to_text "$DETAIL_COMMENT" && shot "$t-09b-post-comments"
  tap_text Back || true
}
